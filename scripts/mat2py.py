...
def prepare_data(data_mat, data_path_src, save_dir, dataset_type='train', generate_npy=False, debug_mode=False, generate_mask_context=False):
    '''
        ...
        for person in range(nop):
            ...
            try:
                ...
                if not os.path.exists(image_path):
                    ...
                    mask_context = context.copy()
                    mask_context[et.bbox[1]:et.bbox[3],et.bbox[0]:et.bbox[2]] = 0
                    context_cv = cv2.resize(context, (224,224))
                    mask_context_cv = cv2.resize(mask_context, (224,224))
            ...
            if generate_npy == True:
                ...
                mask_context_arr.append(mask_context_cv)
                if dataset_type == 'train':
    ...
    if generate_npy:
        if generate_mask_context:
            mask_context_arr = np.array(mask_context_arr)
            print(len(data_set), mask_context_arr.shape)
            np.save(os.path.join(save_dir, '%s_mask_context_arr.npy' % (dataset_type)), mask_context_arr)
            print('completed generating mask context')
            return
...
def generate_mask_context(data_mat, data_path_src, save_dir, dataset_type='train', generate_npy=False, debug_mode=False):
    '''
    Prepare csv files and save preprocessed data in npy files.
    :param data_mat: Mat data object for a label.
    :param data_path_src: Path of the parent directory containing the emotic images folders (mscoco, framesdb, emodb_small, ade20k)
        ...
        for person in range(nop):
            ...
            try:
                ...
                if not os.path.exists(image_path):
                    ...
                    context = cv2.cvtColor(cv2.imread(image_path),cv2.COLOR_BGR2RGB)
                    # body = context[et.bbox[1]:et.bbox[3],et.bbox[0]:et.bbox[2]].copy()
                    context[et.bbox[1]:et.bbox[3], et.bbox[0]:et.bbox[2]] = 0
                    context_cv = cv2.resize(context, (224,224))
                    # body_cv = cv2.resize(body, (128,128))
            except Exception as e:
                to_break += 1
    ...
    cv2.imshow('mask_context', context_arr[-1])
    if generate_npy == True:
        context_arr = np.array(context_arr)
        print (len(data_set), context_arr.shape)
        np.save(os.path.join(save_dir,'%s_mask_context_arr.npy' %(dataset_type)), context_arr)
        print (context_arr.shape)
    print ('completed generating %s mask context' %(dataset_type))
...
def parse_args():
    ...
    parser.add_argument('--generate_mask_context', action='store_true', help='Generate context without body')
...
if __name__ == '__main__':
    ...
    if args.generate_mask_context:
        for label in labels:
            data_mat = mat[label]
            print ('starting label ', label)
            # generate_mask_context(data_mat, data_path_src, save_path, dataset_type=label, generate_npy=args.generate_npy, debug_mode=args.debug_mode)
            prepare_data(data_mat, data_path_src, save_path, dataset_type=label, generate_npy=args.generate_npy,
                         debug_mode=args.debug_mode, generate_mask_context=args.generate_mask_context)
    else:
        for label in labels:
            data_mat = mat[label]
            print ('starting label ', label)
