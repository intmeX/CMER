...
class EmoticTripleStreamDataset(Dataset):
    ''' Triple Input Emotic dataset class. Use preprocessed data stored in npy files. '''
    ...
    def __init__(self, x_context, x_body, x_face, y_cat, y_cont, context_transform, body_transform, face_transform, context_norm, body_norm, face_norm):
        super(EmoticTripleStreamDataset, self).__init__()
        self.x_context = x_context
        self.x_body = x_body
        self.x_face = x_face
        self.y_cat = y_cat
    ...
    def __getitem__(self, index):
        ...
        cont_label = self.y_cont[index]
        return (
            self.context_norm(self.context_transform(image_context)),
            self.body_norm(self.body_transform(image_body)),
            self.face_norm(self.face_transform(image_face)),
